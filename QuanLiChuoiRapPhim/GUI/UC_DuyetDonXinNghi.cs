...
namespace QuanLiChuoiRapPhim.GUI
{
    public partial class UC_DuyetDonXinNghi : UserControl
    {
        ...
        private void KhoiTaoGiaoDien()
        {
            ...
            tabChoDuyet = new TabPage("CHO DUYỆT");
            tabDaDuyet = new TabPage("ĐỢI DUYỆT");
            ...
            {
                Text = $"Tổng đơn: 0 | Cho duyệt: 0 | Đã duyệt: 0 | Từ chối: 0",
        ...
        private void TaoPanelChiTiet()
        {
            ...
            // Ngư?i thay th? (ch? hi?n v?i đơn ch? duy?t)
            ...
            btnTuChoi = new Button
            {
                ...
                BackColor = Color.FromArgb(244, 67, 54), // Đ?
                ForeColor = Color.White,
                FlatStyle = FlatStyle.Flat,
                Font = new Font("Segoe UI", 10, FontStyle.Bold),
                Visible = false
        ...
        private void TaoTabThongKe()
        {
            ...
            dgvThongKe.Columns.Add("ChiTieu", "CHI TIÊU");
        ...
        private void TaiDanhSachNhanVien()
        {
            ...
            {
                MessageBox.Show("Lỗi tại danh sách nhân viên: " + ex.Message, "Lỗi",
        ...
        private void TaiDanhSachDon()
        {
            ...
            {
                MessageBox.Show("Lỗi tại danh sách đơn: " + ex.Message, "Lỗi",
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
            ...
            foreach (Control ctrl in this.Controls)
            {
                if (ctrl is Panel panel && panel.Dock == DockStyle.Bottom)
                {
                    foreach (Control child in panel.Controls)
                    {
                        ...
                        {
                            child.Text = $"Tổng số đơn: {tongDon} | Cho duyệt: {choDuyet} | Đã duyệt: {daDuyet} | Từ chối: {tuChoi} | Đã nghỉ: {daNghi}";
                            break;
                        }
                    }
                    break;
        ...
        private Color GetColorForStatus(string status)
        {
            return status switch
            {
                ...
                " Từ chối" => Color.FromArgb(244, 67, 54),     // Đ?
                " Đã nghỉ" => Color.FromArgb(33, 150, 243),    // Xanh dương
                " Đã hủy" => Color.FromArgb(158, 158, 158),  // Xám
                _ => Color.Gray
            };
        }
        ...
        private void HienThiChiTietDon()
        {
            ...
            lblNguoiDuyet.Text = row["TenNguoiDuyet"] != DBNull.Value ? row["TenNguoiDuyet"].ToString() : "Chưa duy?t";
            lblNgayGui.Text = Convert.ToDateTime(row["NgayGui"]).ToString("dd/MM/yyyy HH:mm");
            lblNgayDuyet.Text = row["NgayDuyet"] != DBNull.Value ?
                Convert.ToDateTime(row["NgayDuyet"]).ToString("dd/MM/yyyy HH:mm") : "Chưa duy?t";
        ...
        private void BtnXemFile_Click(object sender, EventArgs e)
        {
            if (lnkFileDinhKem.Tag != null)
            {
                ...
                {
                    MessageBox.Show("File không tồn tại!", "Lại",
                        MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        }
